...
///
/// This abstract class extends [AbstractConstructorDeclarationSupport] to provide a
...
///
/// ## Constructor
/// The constructor forwards all configuration to [AbstractConstructorDeclarationSupport]
...
/// {@endtemplate}
abstract class AbstractMixinDeclarationSupport extends AbstractConstructorDeclarationSupport {
  ...
  /// ```
  @protected
  MixinDeclaration generateMixin(mirrors.ClassMirror mixinMirror, String libraryUri, Uri sourceUri, bool isBuiltIn) {
    ...
    final result = RuntimeBuilder.timeExecution(() {
      final analyedMixin = getAnalyzedMixinDeclaration(mixinName, sourceUri);
      // final dartType = analyedMixin?.thisType;
      final annotations = extractAnnotations(mixinMirror.metadata, libraryUri, sourceUri, analyedMixin?.metadata);
      type = resolveGenericAnnotationIfNeeded(type, mixinMirror, libraryUri, sourceUri, mixinName);
      ...
      StandardMixinDeclaration reflectedMixin = StandardMixinDeclaration(
        name: mixinName,
        type: type,
        qualifiedName: buildQualifiedName(mixinName, (findRealClassUriFromMirror(mixinMirror) ?? Uri.parse(libraryUri)).toString()),
        library: getLibrary(libraryUri),
        typeArguments: extractTypeVariableAsLinks(mixinMirror.typeVariables, analyedMixin?.typeParameters, libraryUri),
        annotations: annotations,
        sourceLocation: sourceUri,
        constraints: extractMixinConstraintsAsLink(mixinMirror, analyedMixin?.onClause, libraryUri),
        superClass: extractSupertypeAsLink(mixinMirror, null, libraryUri),
        interfaces: extractInterfacesAsLink(mixinMirror, analyedMixin?.implementsClause, libraryUri),
        mixins: extractMixinsAsLink(mixinMirror, null, libraryUri),
      ...
      for (final field in mixinMirror.declarations.values.whereType<mirrors.VariableMirror>()) {
        fields.add(generateField(field, analyedMixin?.members, libraryUri, sourceUri, mixinName, null, null, isBuiltIn));
      }

      // Process methods
      for (final method in mixinMirror.declarations.values.whereType<mirrors.MethodMirror>()) {
        if (!method.isConstructor) {
          methods.add(generateMethod(method, analyedMixin?.members, libraryUri, sourceUri, mixinName, null));
        }
      }
